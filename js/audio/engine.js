...
AudioEngine.prototype.init = function() {
    ...
    try {
        ...
        this.delayGain.gain.value = this.settings.delay * 0.3;
        this.delayGain.connect(this.masterGain);
        
        this.chorusGain = this.audioContext.createGain();
        this.chorusGain.gain.value = this.settings.chorus * 0.3;
...
AudioEngine.prototype.setDelay = function(value) {
    this.settings.delay = Math.max(0, Math.min(1, value));
    if (this.delayGain) {
        this.delayGain.gain.value = this.settings.delay * 0.3;
    }
};

AudioEngine.prototype.setChorus = function(value) {
    this.settings.chorus = Math.max(0, Math.min(1, value));
    if (this.chorusGain) {
        this.chorusGain.gain.value = this.settings.chorus * 0.3;
    }
};
