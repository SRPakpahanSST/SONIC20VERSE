...
KeyboardRenderer.prototype.init = function(audioEngine) {
    ...
    if (octaveSelect) this.currentOctave = parseInt(octaveSelect.value);
...
KeyboardRenderer.prototype.render = function() {
    ...
    allNotes.forEach(function(noteName) {
        ...
        var isWhite = window.isWhiteKey ? window.isWhiteKey(note) : (index % 2 === 0);
        ...
        key.addEventListener('mouseup', function() { self.deactivateKey(this); });
        key.addEventListener('mouseleave', function() { self.deactivateKey(this); });
...
KeyboardRenderer.prototype.bindEvents = function() {
    ...
    document.addEventListener('keydown', function(e) {
        var keyMap = {'q': 0, 'w': 1, 'e': 2, 'r': 3, 't': 4, 'y': 5, 'u': 6, 'i': 7, 'o': 8, 'p': 9, 'a': 10, 's': 11, 'd': 12, 'f': 13, 'g': 14, 'h': 15, 'j': 16, 'k': 17, 'l': 18, ';': 19};
...
KeyboardRenderer.prototype.activateKey = function(key) {
    ...
    if (window.displayManager) {
        window.displayManager.updateNowPlaying(noteName, freq);
    }
};
